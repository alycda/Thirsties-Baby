...
class zipcode_class {
   ...
   function get_zip_details($zip) {
      ...
      $sql = "SELECT zip_latitude AS lattitude, zip_longitude AS longitude, zip_city AS city, zip_county_name AS county, zip_state AS state_name, zip_area_code AS area_code, zip_time_zipe AS time_zone
              FROM zip_code 
              WHERE zip_code=".(int)$zip."
							LIMIT 1";
   ...
   function get_zip_point($zip) {
      ...
      $sql = "SELECT zip_latitude AS lat, zip_longitude AS lon from zip_code WHERE zip_code=".(int)$zip." LIMIT 1";
      $r = mysql_query($sql, $wpdb->dbh);
      if (!$r) {
         $this->last_error = mysql_error();
         return false;
   ...
   function get_zips_in_range($zip, $range, $sort=1, $include_base) {
      ...
      $sql = "SELECT zip_code, zip_latitude AS lat, zip_longitude AS lon FROM zip_code ";
      if (!$include_base) $sql .= "WHERE zip_code <> ".(int)$zip." AND ";
      else $sql .= "WHERE "; 
      $sql .= "zip_latitude BETWEEN '".$min_lat."' AND '".$max_lat."' 
               AND zip_longitude BETWEEN '".$min_lon."' AND '".$max_lon."'";
